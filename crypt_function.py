...
def sign(token):   
    with open("cert/cert.pem","r") as certif:
...
def generate_unique_diploma(user,diploma):
    ...
    fnt = ImageFont.truetype('assets/AlgerianRegular.ttf', 50)
    ...
    img=Image.open("Diplomas/diploma_"+str(diploma._id)+'.png')
    steno.cacher(img,secret_data)
    img.save("Diplomas/diploma_"+str(diploma._id)+'.png')
...
def decrypt_img(filename):
    ...
    return [verifie(encryp_me).decode(),steno.recuperer(diploma,64)]
